...
**⚠️ 整合性メモ**:
- kk 指示の枚数を合計すると **21 枚**（20 枚目標より +1）
- kk の内訳ヘッダは「SSR 3 / SR 7 / R 5 / N 2 = 17 枚」だが、各行の列挙と一致しない
- **要 kk 確認**: 20 枚に圧縮する場合、以下のいずれかで調整が必要:
  - (a) ロスアラモス ×2 → ×1（SR 構成を 7 枚に合わせる）
  - (b) マンハッタン計画・極秘 ×3 → ×2
  - (c) 科学者 ×2 → ×1
  - (d) 21 枚を正とし、ボス戦特別ルールで 21 枚運用する
- **実装時の最終確認項目として扱う**

#### 同名重複サマリ
- マンハッタン計画系: 5 枚（既存 R ×2 + 極秘 SR ×3）
- ロスアラモス: 2 枚
- 核分裂: 2 枚
- ウラン235: 2 枚（§3.3 から +1）
- プルトニウム239: 2 枚（§3.3 から +1）
- 科学者: 2 枚
- **同名重複合計: 15 枚**（以前の 9 枚から大幅増）
...
| # | 項目 | 推奨 |
|---|---|---|
| 12-A | 21 枚 vs 20 枚の枚数調整（§12.2 の整合性メモ） | (a) ロスアラモス ×1 が最有力 |
