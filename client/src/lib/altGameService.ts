...
export async function finalizeAltGame(params: {
  ...
  const { childId, gameType, rawAltEarned, score, maxLevel, maxCombo } = params;
  ...
  await saveAltGameScore({ childId, gameType, score, maxLevel, maxCombo, altEarned });
