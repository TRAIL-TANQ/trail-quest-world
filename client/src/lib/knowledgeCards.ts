...
export const EFFECT_DEFS: Record<string, CardEffect> = {
  ...
  mikka_tenka:   { id: 'mikka_tenka',   name: '儚き天下',         description: '公開時（任意発動）、ベンチの明智光秀をデッキトップに戻す。', category: 'bench' },
  ...
  nobunaga:   { id: 'nobunaga',   name: '天下布武',           description: 'ベンチに「鉄砲」で攻撃+3。「楽市楽座」で防御+2。両方あれば相手ベンチ1枚封印。公開時（自動）、ベンチに本能寺の変があればデッキボトムに戻す。[任意] ベンチに足軽があれば、sub-battle 1回のみ足軽1枚をデッキトップへ戻せる（信長の号令）。', category: 'special' },
  ...
  terracotta:      { id: 'terracotta',      name: '不滅の軍勢',       description: '公開時（任意発動）: 除外の秦の兵士1枚をデッキトップに戻す。', category: 'bench' },
  qinshi:          { id: 'qinshi',          name: '天下統一',         description: '防御時、ベンチに万里の長城があれば防御+2。公開時（自動）、ベンチに焚書坑儒があればデッキトップに戻す。', category: 'def' },
  great_wall:      { id: 'great_wall',      name: '鉄壁の防壁',       description: 'From the bench: 各サブバトルで相手の最初の攻撃カードの攻撃-1（最低1）。', category: 'def' },
  amazon_river:    { id: 'amazon_river',    name: '密林の大河',       description: 'From the bench: アマゾン生物の防御+1。', category: 'def' },
  anaconda:        { id: 'anaconda',        name: '締めつけ',         description: 'From the bench: 相手の防御バフを-2する（素の防御は減らない、重複可）。', category: 'debuff' },
  poison_frog:     { id: 'poison_frog',     name: '猛毒',             description: '防御時（任意発動）、相手ベンチのカード1枚を除外する。', category: 'debuff' },
  ...
  stackable_weapon_buff: { id: 'stackable_weapon_buff', name: '武器の熟練', description: '攻撃時、自分のスタック（attackRevealed）に同名カード1枚につき攻撃+1（1枚目はバフなし）。サブバトル終了でリセット。', category: 'atk' },
  rakuichi_rakuza_recycle: { id: 'rakuichi_rakuza_recycle', name: '自由市場の再装填', description: '公開時、ベンチの鉄砲1枚をデッキトップへ。ベンチに織田信長がいれば、足軽1枚もデッキ2番目へ。From the bench: 信長の防御+2。', category: 'def' },
  nagashino:       { id: 'nagashino',       name: '三段撃ちの陣',     description: '公開時、ベンチの鉄砲が3枚以上で攻撃+2（固定値）。', category: 'atk' },
  bafousaku:       { id: 'bafousaku',       name: '馬防柵の構え',     description: '公開時：守りを宣言。From the bench: 各サブバトル相手の最初の攻撃カードの攻撃-1（最低1）。', category: 'def' },
  ...
  book_burning:     { id: 'book_burning',     name: '思想統制',         description: '公開時、自ベンチの「紙＋始皇帝の勅令」の合計枚数 X。相手と自分それぞれのデッキトップから X 枚を除外する。', category: 'debuff' },
  elixir:           { id: 'elixir',           name: '永遠の命',         description: '攻撃時（1バトル1回）: 除外の始皇帝をデッキトップに戻す。', category: 'special' },
  giant_snake:      { id: 'giant_snake',      name: '呑み込む者',       description: 'ベンチにある攻撃力2のカード1枚につき攻撃+1/防御+1。', category: 'special' },
  photosynthesis:   { id: 'photosynthesis',   name: '密林の再生',       description: '公開時（任意発動）、デッキから植物カード1枚をデッキトップに置く。', category: 'bench' },
  anaconda_hunter:  { id: 'anaconda_hunter',  name: '蛇の巫師',         description: '公開時（任意発動）、ベンチにアマゾン川+アナコンダがあれば、アナコンダ1枚を除外してデッキ外の大蛇をデッキトップに召喚。', category: 'special' },
  ...
  sei_shonagon:     { id: 'sei_shonagon',     name: '鋭敏の守護',       description: '交戦中、ベンチの十二単の枚数だけ防御+1。', category: 'def' },
  genji:            { id: 'genji',            name: '物語の魔力',       description: '公開時（任意発動）、ベンチの和歌の枚数まで自分のベンチのカードを選んでデッキに戻す。', category: 'bench' },
  makura_no_soshi:  { id: 'makura_no_soshi',  name: '随筆の鋭刃',       description: '公開時（任意発動）、ベンチの筆の枚数まで相手のベンチのカードを選んで除外する。', category: 'special' },
  // ===== オオカミデッキ =====
...
export const EFFECT_BY_CARD_NAME: Record<string, string> = {
  ...
  'キャラベル船':         'caravel',
  '香辛料':               'spice',
  '鉄砲':                 'stackable_weapon_buff',
  '楽市楽座':             'rakuichi_rakuza_recycle',
  '長篠の陣':             'nagashino',
  '馬防柵':               'bafousaku',
  '豊臣秀吉':             'hideyoshi',
  '千利休':               'rikyu',
...
export const CARD_STAT_OVERRIDES: Record<string, StatProfile> = {
  ...
  '毒矢カエル':       { attackPower: 1, defensePower: 2 },
  ...
  'ジャンヌ・ダルク': { attackPower: 2, defensePower: 2 },
  '軍旗':             { attackPower: 1, defensePower: 1 },
  'ケーキ':           { attackPower: 1, defensePower: 1 },
  '万里の長城':       { attackPower: 1, defensePower: 4 },
  '兵馬俑':           { attackPower: 2, defensePower: 2 },
  // ===== コンボ系 第2弾 =====
  ...
  '不老不死の薬':         { attackPower: 1, defensePower: 3 },
  '大蛇':                 { attackPower: 4, defensePower: 3 },
  '光合成':               { attackPower: 1, defensePower: 1 },
  '大蛇の巫師':           { attackPower: 3, defensePower: 3 },
  'ジャガー':             { attackPower: 3, defensePower: 1 },
  'ピンクイルカ':         { attackPower: 1, defensePower: 2 },
  '凱旋門':               { attackPower: 1, defensePower: 3 },
  '秦の兵士':             { attackPower: 2, defensePower: 1 },
  '始皇帝の勅令':         { attackPower: 1, defensePower: 2 },
  // ===== ジャンヌダルクデッキ =====
  '祈りの光':             { attackPower: 1, defensePower: 1 },
  '白百合の盾':           { attackPower: 1, defensePower: 2 },
  '聖女の旗印':           { attackPower: 2, defensePower: 2 },
  '聖女ジャンヌ':         { attackPower: 3, defensePower: 6 },
  '火刑':                 { attackPower: 1, defensePower: 3 },
  // ===== 紫式部・清少納言デッキ =====
  '紫式部':               { attackPower: 2, defensePower: 3 },
  '清少納言':             { attackPower: 3, defensePower: 2 },
  '源氏物語':             { attackPower: 2, defensePower: 5 },
  '枕草子':               { attackPower: 5, defensePower: 2 },
  '筆':                   { attackPower: 1, defensePower: 1 },
  '和歌':                 { attackPower: 1, defensePower: 1 },
...
// ===== Deck Building Rules =====
// Initial deck = 15 cards. Max deck = 15. Minimum during trim = 6 (MIN_DECK_SIZE).
...
export const SYNERGY_MAP: Record<string, string[]> = {
  ...
  // 安土桃山コンボ
  '織田信長': ['鉄砲', '楽市楽座', '千利休', '豊臣秀吉', '本能寺の変'],
  '本能寺の変': ['織田信長', '明智光秀', '愛宕百韻', '天王山', '三日天下'],
  '明智光秀': ['愛宕百韻', '天王山', '三日天下', '本能寺の変'],
  '愛宕百韻': ['明智光秀', '天王山', '三日天下'],
  '天王山': ['明智光秀', '愛宕百韻', '三日天下'],
  '三日天下': ['明智光秀', '愛宕百韻', '天王山'],
  ...
  '秦の兵士': ['万里の長城', '兵馬俑', '始皇帝'],
  '始皇帝の勅令': ['紙', '焚書坑儒', '始皇帝'],
  'アマゾン川': ['アナコンダ', '毒矢カエル', 'ピラニア', 'ジャガー', 'ピンクイルカ', '大蛇', '大蛇の巫師'],
  'ピラニア': ['アマゾン川', 'アナコンダ', '毒矢カエル', 'ジャガー', 'ピンクイルカ', 'ダーウィン'],
  'アナコンダ': ['アマゾン川', '毒矢カエル', 'ピラニア', 'ジャガー', 'ピンクイルカ', 'ダーウィン', '大蛇', '大蛇の巫師'],
  '大蛇の巫師': ['アナコンダ', 'アマゾン川', 'ピンクイルカ', '大蛇'],
  '大蛇': ['アマゾン川', 'アナコンダ', '毒矢カエル', 'ジャガー', 'ピンクイルカ', '大蛇の巫師'],
  '毒矢カエル': ['アマゾン川', 'アナコンダ', 'ピラニア', 'ジャガー', 'ピンクイルカ'],
  'ジャガー': ['アマゾン川', 'アナコンダ', 'ピラニア', '毒矢カエル', 'ピンクイルカ'],
  'ピンクイルカ': ['アマゾン川', '大蛇の巫師', 'アナコンダ', 'ピラニア', '毒矢カエル', 'ジャガー'],
  'ニュートン': ['リンゴ', 'プリズム', '万有引力'],
  'リンゴ': ['ニュートン', 'プリズム', '万有引力'],
  'プリズム': ['ニュートン', 'リンゴ', '万有引力'],
  '万有引力': ['ニュートン', 'リンゴ', 'プリズム'],
