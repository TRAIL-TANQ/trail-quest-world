...
//
// 2026-04-22: 0033 (child_status) + 0036 (quiz_attempts ほか) で全テーブルの
// child_id が text 型に統一された。従来の UUID ガード (isUuid) は撤廃済み。
...
export async function updateChildStatus(
  ...
  if (isGuest() || isAdmin()) {
    // Guest/Admin mode: local-only update via cache
