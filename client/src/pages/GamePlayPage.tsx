 ...
 * Rich gold frames, potion bottles, magical atmosphere
 */
import { useState, useEffect, useCallback, useRef } from 'react';
import { useParams, useLocation } from 'wouter';
import { useUserStore, useGameStore } from '@/lib/stores';
...
// Tile colors for the number grid
const tileColors = [
...
export default function GamePlayPage() {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background: '#0b1128' }}>
        <div className="text-center">
          <p className="gold-text text-xl mb-4">ゲームが見つかりません</p>
      ...
      >
        ...
        >
          ...
          <div className="flex items-center justify-center gap-5 mb-6">
            {[
              { label: '難易度', content: Array.from({ length: 5 }).map((_, i) => (
                <span key={i} className="text-xs" style={{ color: i < game.difficulty ? '#ffd700' : 'rgba(255,255,255,0.12)' }}>★</span>
              ))},
              { label: '報酬', content: <span className="text-sm font-bold" style={{ color: '#ffd700' }}>+{game.altReward} ALT</span> },
              { label: '制限時間', content: <span className="text-sm font-bold text-amber-100">60秒</span> },
            ].map((item, i) => (
              <div key={i} className="text-center">
                <p className="text-[9px] text-amber-200/35 mb-1">{item.label}</p>
                <div className="flex gap-0.5 justify-center">{item.content}</div>
              </div>
            ))}
          </div>
  ...
  return (
    ...
    >
      ...
      >
        <button onClick={() => navigate('/games')} className="text-amber-200/35 text-sm hover:text-amber-200/60 transition-colors">
          ✕
        </button>
        <div className="text-center">
