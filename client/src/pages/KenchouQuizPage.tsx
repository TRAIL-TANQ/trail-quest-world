/**
 * KenchouQuizPage — 🏙️ 県庁所在地クイズ
 *
 * 県名 → 県庁所在地 を4択で答える（★1-★4）。
 * ★5 は逆引き: 県庁所在地 → 県名 を4択で答える。
 *
 * ★3+ では同県の他都市を選択肢に混ぜて紛らわしくする。
 */
...
import { KENCHOU_DATA, getKenchouByDifficulty, type KenchouQuestion } from '@/data/kenchouData';
...
interface DiffConfig {
  ...
  mixOtherCities: boolean; // ★3+: 同県の他都市を選択肢に混ぜる
  reverseMode: boolean;    // ★5: 県庁所在地→県名
}

const GAME_TYPE = 'kenchou_quiz' as const;
const BEST_KEY_PREFIX = 'kenchou_best_';

const DIFF_CONFIGS: Record<DiffId, DiffConfig> = {
  1: { id: 1, label: 'かんたん',   timeSeconds: 60, altPerCorrect: 1, mixOtherCities: false, reverseMode: false, comboTiers: [] },
  2: { id: 2, label: 'ふつう',     timeSeconds: 60, altPerCorrect: 1, mixOtherCities: false, reverseMode: false, comboTiers: [
    { threshold: 5, multiplier: 2, label: '🔥 コンボ ×2' },
  ] },
  3: { id: 3, label: 'むずかしい', timeSeconds: 60, altPerCorrect: 2, mixOtherCities: true,  reverseMode: false, comboTiers: [
    { threshold: 5, multiplier: 2, label: '🔥 コンボ ×2' },
    { threshold: 10, multiplier: 3, label: '⚡ スーパーコンボ ×3' },
  ] },
  4: { id: 4, label: 'ゲキむず',   timeSeconds: 45, altPerCorrect: 3, mixOtherCities: false, reverseMode: false, comboTiers: [
    { threshold: 5, multiplier: 2, label: '🔥 コンボ ×2' },
    { threshold: 10, multiplier: 3, label: '⚡ スーパーコンボ ×3' },
  ] },
  5: { id: 5, label: '鬼',         timeSeconds: 30, altPerCorrect: 5, mixOtherCities: false, reverseMode: true, comboTiers: [
    { threshold: 5, multiplier: 2, label: '🔥 コンボ ×2' },
    { threshold: 10, multiplier: 3, label: '⚡ スーパーコンボ ×3' },
...
function makeProblem(q: KenchouQuestion, cfg: DiffConfig, pool: KenchouQuestion[]): Problem {
  if (cfg.reverseMode) {
    // 県庁所在地→県名（4択は他県名）
    const others = shuffle(pool.filter((p) => p.prefecture !== q.prefecture)).slice(0, 3);
    const choices = shuffle(uniq([q.prefecture, ...others.map((p) => p.prefecture)])).slice(0, 4);
    return { q, question: q.capital, choices, correctIndex: choices.indexOf(q.prefecture) };
  }
  // 県名→県庁所在地
  // ★4（ゲキむず）は他県の県庁所在地を主な紛らわしい選択肢に
  // ★3 は otherCities（同県別都市）を混ぜる
  let distractors: string[];
  if (cfg.mixOtherCities) {
    // 同県の他都市 2つ + 他県の県庁所在地 1つ
    const sameCounty = shuffle(q.otherCities).slice(0, 2);
    const other = shuffle(pool.filter((p) => p.prefecture !== q.prefecture)).slice(0, 1);
    distractors = [...sameCounty, ...other.map((p) => p.capital)];
  } else {
    // 他県の県庁所在地3つ
    const others = shuffle(pool.filter((p) => p.prefecture !== q.prefecture)).slice(0, 3);
    distractors = others.map((p) => p.capital);
  }
  const choices = shuffle(uniq([q.capital, ...distractors])).slice(0, 4);
  // 足りない場合は他県から補充
  while (choices.length < 4) {
    const fill = KENCHOU_DATA[Math.floor(Math.random() * KENCHOU_DATA.length)].capital;
    if (!choices.includes(fill)) choices.push(fill);
  }
  return { q, question: q.prefecture, choices, correctIndex: choices.indexOf(q.capital) };
}

function pickNext(pool: KenchouQuestion[], prev: KenchouQuestion | null, cfg: DiffConfig): Problem {
  if (pool.length === 0) {
    const fallback = KENCHOU_DATA[0];
    return makeProblem(fallback, cfg, KENCHOU_DATA);
  }
  let q = pool[Math.floor(Math.random() * pool.length)];
  if (prev && pool.length > 1) {
    for (let i = 0; i < 10 && q.prefecture === prev.prefecture; i++) {
      q = pool[Math.floor(Math.random() * pool.length)];
    }
  }
  return makeProblem(q, cfg, KENCHOU_DATA);
}
...
export default function KenchouQuizPage() {
  ...
  const pool = useMemo(() => getKenchouByDifficulty(activeDiff), [activeDiff]);
  ...
  const startGame = useCallback(() => {
    ...
    setIsNewBest(false);
    const nextPool = getKenchouByDifficulty(selectedDiff);
    setProblem(pickNext(nextPool, null, cfg));
    setPhase('playing');
  }, [selectedDiff]);
  ...
  const handleChoice = useCallback((idx: number) => {
    ...
    if (correct) {
      ...
      feedbackTimerRef.current = window.setTimeout(() => {
        setFeedback(null); setPicked(null); setComboFlash(null);
        setProblem((prev) => pickNext(pool, prev?.q ?? null, cfg));
      }, 300);
    } else {
      playError();
      setFeedback('wrong');
    ...
    return (
      <div className="px-4 pt-6 pb-6 tqw-animate-fadeIn">
        ...
        <div className="tqw-card-panel rounded-2xl p-6 text-center">
          ...
          <h1 className="text-2xl font-black mb-2 tqw-title-game">県庁所在地クイズ</h1>
          <p className="text-sm mb-4" style={{ color: 'var(--tqw-text-gray)' }}>
            都道府県の県庁所在地を4つから選ぼう<br />★5 は逆引き（県庁所在地→県名）
          </p>
  ...
  if (phase === 'playing' && problem) {
    ...
    const correctText = activeCfg.reverseMode ? problem.q.prefecture : problem.q.capital;
    return (
      ...
      >
        ...
        >
          <div className="text-[11px] mb-1" style={{ color: 'var(--tqw-text-gray)' }}>
            {activeCfg.reverseMode ? 'この県庁所在地は何県？' : 'この県の県庁所在地は？'}
          </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          {problem.choices.map((c, i) => {
            ...
            const isCorrectBtn = feedback && c === correctText;
