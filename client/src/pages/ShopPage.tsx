...
export default function ShopPage() {
  ...
  const isAvatarOwned = (avatarId: string) => {
    return user.purchasedAvatarIds.includes(avatarId);
  };

  const isAvatarEquipped = (avatarId: string) => {
    return user.equippedAvatarId === avatarId;
  };

  const handlePurchaseAvatar = (item: typeof MOCK_SHOP_ITEMS[0]) => {
    if (isAvatarOwned(item.id)) {
    ...
    const success = purchaseAvatar(item.id, item.price);
    if (success) {
  ...
  const handleEquipAvatar = (avatarId: string) => {
    if (isAvatarEquipped(avatarId)) {
      // Unequip - go back to default
      equipAvatar(null);
      toast.info('アバターを外しました');
      return;
    }
    equipAvatar(avatarId);
    toast.success('アバターを装備しました！');
  };

  const handlePurchaseOther = (item: typeof MOCK_SHOP_ITEMS[0]) => {
        ...
        {activeTab === 'avatar' && (
          <div className="grid grid-cols-2 gap-3">
            {filteredItems.map((item, i) => {
              const avatarImages = AVATAR_ITEMS[item.id];
              const tabColor = activeTabData?.color || '#ffd700';
              const owned = isAvatarOwned(item.id);
              const equipped = isAvatarEquipped(item.id);
              return (
                <div key={item.id} className="rounded-xl overflow-hidden transition-all duration-200 animate-slide-up card-shine relative"
                  style={{
                    ...
                    border: equipped ? '1.5px solid rgba(255,215,0,0.5)' : owned ? '1.5px solid rgba(34,197,94,0.3)' : `1.5px solid ${tabColor}22`,
                  ...
                  <div className="p-2.5" style={{ borderTop: `1px solid ${equipped ? 'rgba(255,215,0,0.2)' : owned ? 'rgba(34,197,94,0.15)' : tabColor + '15'}` }}>
                    ...
                    {owned ? (
                      ...
                      <button onClick={() => handlePurchaseAvatar(item)}
                        className="w-full py-1.5 rounded-lg text-[11px] font-bold transition-all active:scale-95"
                        style={{
              ...
              return (
                <div key={item.id} className="rounded-xl overflow-hidden transition-all duration-200 animate-slide-up card-shine relative"
                  ...
                  <div className="p-2.5" style={{ borderTop: `1px solid ${item.owned ? 'rgba(34,197,94,0.15)' : tabColor + '15'}` }}>
                    <p className="text-xs font-bold text-amber-100 mb-1 truncate">{item.name}</p>
                    <p className="text-[10px] text-amber-200/35 mb-2 line-clamp-1">{item.description}</p>
                    <button onClick={() => handlePurchaseOther(item)} disabled={item.owned}
